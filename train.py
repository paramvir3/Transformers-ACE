...
def _limit_neighbors(edge_index: torch.Tensor, num_nodes: int, max_neighbors: int) -> torch.Tensor:
    if max_neighbors <= 0 or edge_index.numel() == 0:
        return edge_index
    sender, receiver = edge_index
    perm = receiver.argsort()
    receiver_sorted = receiver[perm]
    counts = torch.bincount(receiver_sorted, minlength=num_nodes)
    offsets = torch.cumsum(counts, dim=0)
    starts = offsets - counts
    idx_in_group = torch.arange(receiver_sorted.numel(), device=receiver.device) - starts[receiver_sorted]
    keep_sorted = idx_in_group < max_neighbors
...
def save_checkpoint(path, epoch, model, optimizer, scheduler, scaler, config, energy_shift_per_atom, atomic_energies):
    ...
    checkpoint = {
        ...
        'config': {
            ...
            'pt_pos_hidden': config.get('pt_pos_hidden', 64),
            'pt_ffn_hidden': config.get('pt_ffn_hidden', None),
            'pt_dropout': config.get('pt_dropout', 0.0),
            'pt_residual_dropout': config.get('pt_residual_dropout', 0.0),
            'pt_ffn_gated': config.get('pt_ffn_gated', False),
            ...
            'pt_factorized': config.get('pt_factorized', False),
            'pt_l_max': config.get('pt_l_max', None),
            'pt_radial_mlp_hidden': config.get('pt_radial_mlp_hidden', 64),
            'pt_radial_mlp_layers': config.get('pt_radial_mlp_layers', 2),
            'descriptor_backend': config.get('descriptor_backend', 'tace'),
            'tace_atomic_numbers': config.get('tace_atomic_numbers', None),
            'tace_num_layers': config.get('tace_num_layers', 2),
            'tace_Lmax': config.get('tace_Lmax', 1),
            'tace_lmax': config.get('tace_lmax', 1),
            'tace_avg_num_neighbors': config.get('tace_avg_num_neighbors', 64),
            'tace_num_channel': config.get('tace_num_channel', 64),
            'tace_num_channel_hidden': config.get('tace_num_channel_hidden', 64),
            'tace_radial_basis': config.get('tace_radial_basis', None),
            'tace_angular_basis': config.get('tace_angular_basis', None),
            'tace_radial_mlp': config.get('tace_radial_mlp', None),
            'tace_inter': config.get('tace_inter', None),
            'tace_prod': config.get('tace_prod', None),
            'tace_universal_embedding': config.get('tace_universal_embedding', None),
            'pt_equivariant': config.get('pt_equivariant', False),
            'pt_vector_dim': config.get('pt_vector_dim', 32),
            'pt_hierarchical': config.get('pt_hierarchical', False),
            'pt_voxel_size': config.get('pt_voxel_size', 2.0),
            'pt_long_range': config.get('pt_long_range', False),
        ...
        if precompute_neighbors:
            ...
            for atoms in atoms_list:
                ...
                if self.max_neighbors > 0:
                    edge_index = _limit_neighbors(edge_index, len(atoms), self.max_neighbors)
...
def main():
    ...
    train_ds = AtomisticDataset(
        ...
        max_neighbors=config.get('max_neighbors', 0),
    )
