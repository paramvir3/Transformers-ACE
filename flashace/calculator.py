...
class FlashACECalculator(Calculator):
    ...
    def __init__(self, model_path="model.pt", device=None, **kwargs):
        ...
        self.model = FlashACE(
            ...
            pt_pos_hidden=conf.get('pt_pos_hidden', 64),
            pt_ffn_hidden=conf.get('pt_ffn_hidden', None),
            pt_dropout=conf.get('pt_dropout', 0.0),
            pt_residual_dropout=conf.get('pt_residual_dropout', 0.0),
            pt_ffn_gated=conf.get('pt_ffn_gated', False),
            ...
            pt_factorized=conf.get('pt_factorized', False),
            pt_l_max=conf.get('pt_l_max', None),
            pt_radial_mlp_hidden=conf.get('pt_radial_mlp_hidden', 64),
            pt_radial_mlp_layers=conf.get('pt_radial_mlp_layers', 2),
            descriptor_backend=conf.get('descriptor_backend', 'tace'),
            tace_atomic_numbers=conf.get('tace_atomic_numbers', None),
            tace_num_layers=conf.get('tace_num_layers', 2),
            tace_Lmax=conf.get('tace_Lmax', 1),
            tace_lmax=conf.get('tace_lmax', 1),
            tace_avg_num_neighbors=conf.get('tace_avg_num_neighbors', 64),
            tace_num_channel=conf.get('tace_num_channel', 64),
            tace_num_channel_hidden=conf.get('tace_num_channel_hidden', 64),
            tace_radial_basis=conf.get('tace_radial_basis', None),
            tace_angular_basis=conf.get('tace_angular_basis', None),
            tace_radial_mlp=conf.get('tace_radial_mlp', None),
            tace_inter=conf.get('tace_inter', None),
            tace_prod=conf.get('tace_prod', None),
            tace_universal_embedding=conf.get('tace_universal_embedding', None),
            pt_equivariant=conf.get('pt_equivariant', False),
            pt_vector_dim=conf.get('pt_vector_dim', 32),
            pt_hierarchical=conf.get('pt_hierarchical', False),
            pt_voxel_size=conf.get('pt_voxel_size', 2.0),
            pt_long_range=conf.get('pt_long_range', False),
