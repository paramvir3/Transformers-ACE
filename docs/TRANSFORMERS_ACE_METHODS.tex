...
correlations through a configurable maximum body order. Attention queries depend on the
...
recursive Clebsch--Gordan products construct correlations up to configurable
maximum body order $\nu_{\max}$:
\begin{align}
 \vect C_i^{(2)} &= \vect A_i,\\
 \vect C_i^{(\nu+1)} &= \mathrm{TP}_{\nu+1}(\vect C_i^{(\nu)},\vect A_i),
...
not claimed to be a formally complete, untruncated linear ACE basis. The
ACEsuit/EquivariantTensors.jl construction enumerates sparse many-body
$(n,\ell,m)$ specifications, applies symmetric products, and then uses sparse
permutation-invariant Clebsch--Gordan symmetrization maps~\cite{equivarianttensors}.
Transformers-ACE keeps the same $O(3)$ tensor-product rules and neighbor
permutation invariance, but uses a learned low-rank density polynomial for
scalability. Increasing $\ell_{\max}$, radial resolution, multiplicities, and
correlation order expands the representation systematically but changes
computational cost.
...
\begin{itemize}
  ...
  \item internal $O(3)$ equivariance and inversion parity of ACE tensor channels;
...
\bibitem{equivarianttensors}
ACEsuit,
``EquivariantTensors.jl,''
\href{https://github.com/ACEsuit/EquivariantTensors.jl}{github.com/ACEsuit/EquivariantTensors.jl}.
\end{thebibliography}
